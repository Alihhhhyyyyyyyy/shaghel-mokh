import { collection, getDocs, addDoc, doc, setDoc, onSnapshot, query, orderBy, limit, where, deleteDoc, updateDoc, getDoc }
...
window.closeModal = type => {
  $(`modal-${type}`)?.classList.remove('active');
...
window.updateUI = () => {
  ...
  ['home-avatar-frame','side-avatar-frame'].forEach(id=>{
    ...
    if(id==='home-avatar-frame') el.style.borderRadius='44px';
  ...
  const themeIconSb=$('theme-icon-sb'),themeLabelSb=$('theme-label-sb');
  ...
  // Quiz streak badge
  const st2=d.stats?.currentStreak||0;
  const sb=$('quiz-streak-badge'); const sn=$('quiz-streak-num');
  if(sb&&sn){sn.innerText=st2;sb.style.display=st2>=2?'inline-flex':'none';}
...
window.navTo = id => {
  ...
  const hideNav=['quiz','result','lobby'].includes(id);
  $('main-nav').style.display=hideNav?'none':'flex';
  if(id==='map')         renderMap();
...
function renderColorPicker(){
  const container=$('theme-color-picker');
  if(!container) return;
  container.innerHTML='';
...
window.requestNotifPermission = async ()=>{
  ...
  if(perm==='granted'){
    ...
    $('notif-btn').innerText='✅ الإشعارات مفعلة';
    $('notif-btn').style.background='rgba(34,197,94,.1)';
    $('notif-btn').style.color='#22c55e';
...
function renderMap(){
  ...
  const unlocked=Object.keys(categoryConfig);
  const completed=window.gameData.stats?.completedSections||0;
  ...
  keys.forEach((key,i)=>{
    ...
    if(isUnlocked&&!isDone){
      node.onclick=()=>{ selectedCategory=key; showSubsForMap(key); };
    }
...
function showSubsForMap(key){
  ...
  cat.subs.forEach(sub=>{
    ...
    div.style.cssText='background:var(--card);padding:18px 20px;border-radius:22px;' +
      'border:1px solid rgba(255,255,255,.05);display:flex;justify-content:space-between;' +
      'align-items:center;cursor:pointer;transition:.2s';
    ...
    <span style="background:var(--grad);color:#000;padding:8px 18px;border-radius:14px;
      font-weight:900;font-size:12px;border:none">ابدأ</span>`;
...
async function generateAndSave(cat,sub){
  ...
  try{
    const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_KEY}`,{
      ...
      body:JSON.stringify({
        contents:[{parts:[{text:`أنشئ 10 أسئلة MCQ عربية عن "${cat}-${sub}". JSON فقط: [{"t":"...","a":["...","...","...","..."],"c":0,"x":"..."}]`}]}]
      })
    });
...
function showQuestion(){
  ...
  q.a.forEach((opt,i)=>{
    ...
    btn.className='btn-option shimmer';
    btn.innerText=opt;
...
function selectAnswer(i,btn){
  ...
  if(i===q.c){
    ...
    updateDailyTask('win_5',1); updateDailyTask('earn_500',earned);
    ...
    if(document.querySelectorAll('.btn-option')[q.c]) document.querySelectorAll('.btn-option')[q.c].classList.add('correct');
...
async function finishQuiz(){
  ...
  if(isDailyChallenge){
    ...
    if(window.firebaseReady&&window.currentUser){
      await window.db_set(
        ...
        {username:window.gameData.username,avatar:window.gameData.avatar,
         score:quizCorrect,uid:window.currentUser.uid,ts:Date.now()}, true
      );
    }
  }
  ...
  $('result-emoji').innerText=emoji; $('result-title').innerText=title;
  $('result-subtitle').innerText=`${pct}% إجابات صحيحة`;
  $('res-correct').innerText=quizCorrect; $('res-wrong').innerText=quizWrong;
  $('res-coins').innerText=`+${quizCoins} 💰`; $('res-xp').innerText=`+${quizXP} XP`;
...
window.renderLeaderboard = async (tab='global')=>{
  ...
  try{
    ...
    const topBorder=['rgba(251,191,36,.3)','rgba(203,213,225,.2)','rgba(217,119,6,.2)'];
    leaders.forEach((u,i)=>{
      const rank=i+1;
      const isMe=u.uid===window.currentUser?.uid;
      const el=document.createElement('div');
      el.className=`leader-item${isMe?' me':''}`;
      if(rank<=3){el.style.background=topBg[rank-1];el.style.borderColor=topBorder[rank-1];}
      const score=tab==='season'?(u[`season_${season}`]||0):tab==='daily'?`${u.score||0}/10`:(u.xp||0);
      const scoreLabel=tab==='daily'?'نقطة':'XP';
      const frame=AVATAR_FRAMES.find(f=>f.id===(u.avatarFrame||'none'))||AVATAR_FRAMES[0];
      ...
      el.innerHTML=`
        ...
        <div style="position:relative;flex-shrink:0">
          <img src="${u.avatar||'https://i.postimg.cc/qqTBP312/1000061201.png'}"
            ...
            border:2px solid ${isMe?accentCol:'rgba(255,255,255,.08)'};display:block">
        </div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:900;font-size:13px;color:${isMe?accentCol:'#fff'};
...
async function renderDailyChallenge(){
  ...
  if(header){
    ...
    header.innerHTML=done?`
      ...
      <div class="daily-countdown" style="font-size:14px;color:var(--text2);margin-top:8px">
        التحدي القادم بعد ${hh}:${mm}:${ss}
      </div>`:`
      <div class="daily-date">تحدي ${todayISO}</div>
      ...
      <button onclick="window.startDailyChallenge()"
        style="margin-top:14px;background:var(--grad);color:#000;border:none;
        border-radius:18px;padding:13px 32px;font-weight:900;font-size:15px;
        cursor:pointer;font-family:'Tajawal',sans-serif;border-bottom:3px solid rgba(0,0,0,.2)">
        ابدأ التحدي 🎯
      </button>`;
  }
  // daily leaderboard
      ...
      try{
        ...
        rows.slice(0,10).forEach((u,i)=>{
          ...
          ldr.innerHTML+=`<div class="leader-item${isMe?' me':''}">
            <div style="width:28px;height:28px;border-radius:9px;background:#1e1e1e;
              display:flex;align-items:center;justify-content:center;font-weight:900;font-size:12px">${i+1}</div>
            <img src="${u.avatar||'https://i.postimg.cc/qqTBP312/1000061201.png'}"
              style="width:38px;height:38px;border-radius:12px;object-fit:cover;flex-shrink:0">
...
window.startDailyChallenge = async ()=>{
  // Use seeded random based on today's date for consistent questions
  const today=new Date().toISOString().slice(0,10);
...
window.confirmCreateRoom = async ()=>{
  ...
  const roomData={
    ...
    players:{[window.currentUser.uid]:{
      uid:window.currentUser.uid, username:window.gameData.username,
      avatar:window.gameData.avatar, ready:false, score:0
    }},
    createdAt:Date.now()
  };
  try{
...
window.joinRoomById = async roomId=>{
  ...
  await updateDoc(roomRef,{[`players.${window.currentUser.uid}`]:{
    uid:window.currentUser.uid, username:window.gameData.username,
    avatar:window.gameData.avatar, ready:false, score:0
  }});
  window.navTo('lobby');
...
function listenLobby(roomId){
  ...
  roomUnsubscribe=onSnapshot(ref,snap=>{
    ...
    if(startBtn) startBtn.style.display=isHost&&playerCount>=2?'block':'none';
    if(waitMsg)  waitMsg.style.display=isHost&&playerCount>=2?'none':'block';
    if(room.status==='playing'&&!isHost){
      startRoomGameAsPlayer(room);
    }
  });
}
...
window.startRoomGame = async ()=>{
  ...
  const pool=await fetchQuestions(
    categoryConfig[Object.keys(categoryConfig)[0]].name,
    categoryConfig[Object.keys(categoryConfig)[0]].subs[0]
  );
...
async function startRoomGameAsPlayer(room){
...
function loadRooms(){
  ...
  roomsUnsubscribe=onSnapshot(
    query(collection(window.db,'artifacts',window.appId,'public','data','rooms'),
          where('status','==','waiting'),orderBy('createdAt','desc'),limit(10)),
    snap=>{
      list.innerHTML='';
      if(snap.empty){
        list.innerHTML=`<div style="text-align:center;padding:40px;opacity:.4;font-weight:700">
          ...
          لا توجد غرف نشطة<br>
          <span style="font-size:13px">أنشئ غرفة جديدة وادعُ أصحابك!</span>
        </div>`;
        return;
      }
      snap.forEach(d=>{
        const r=d.data(); const pc=Object.keys(r.players||{}).length;
        const el=document.createElement('div');
        ...
        <button onclick="window.joinRoomById('${d.id}')"
          style="background:var(--grad);color:#000;border:none;border-radius:14px;
          padding:10px 18px;font-weight:900;font-size:13px;cursor:pointer;
          font-family:'Tajawal',sans-serif;white-space:nowrap;border-bottom:2px solid rgba(0,0,0,.2)">انضمام</button>`;
        list.appendChild(el);
      });
      // Add join by code button at bottom
      list.innerHTML+=`<button onclick="window.closeModal('join-room');openModal('join-room')"
        style="width:100%;margin-top:8px;padding:14px;background:rgba(255,255,255,.05);
        color:var(--text2);border:1px solid rgba(255,255,255,.08);border-radius:18px;
        font-weight:900;font-size:14px;cursor:pointer;font-family:'Tajawal',sans-serif"
        onclick="openModal('join-room')">🔑 الانضمام بكود</button>`;
    }
  );
}
...
function shopItem(icon,name,sub,price,fn){
  return `<div style="background:rgba(24,24,27,.5);padding:20px;border-radius:28px;
    border:1px solid rgba(255,255,255,.06);display:flex;justify-content:space-between;
    align-items:center;margin-bottom:12px">
    <div style="display:flex;align-items:center;gap:14px">
      <div style="width:52px;height:52px;background:rgba(251,191,36,.1);border-radius:18px;
        display:flex;align-items:center;justify-content:center;font-size:26px">${icon}</div>
      <div>
        <h4 style="font-weight:900;font-size:16px;margin-bottom:2px">${name}</h4>
        <p style="font-size:10px;opacity:.4;font-weight:700;text-transform:uppercase;letter-spacing:.1em">${sub}</p>
      </div>
    </div>
    <button onclick="${fn}" style="background:var(--grad);color:#000;padding:10px 18px;
      border-radius:14px;font-weight:900;border:none;border-bottom:3px solid rgba(0,0,0,.2);
      cursor:pointer;font-family:'Tajawal',sans-serif;font-size:13px;white-space:nowrap">${price} 💰</button>
  </div>`;
}
...
function freeCoinsItem(){
  return `<div style="background:rgba(24,24,27,.5);padding:20px;border-radius:28px;
    ...
    align-items:center;margin-bottom:12px">
    <div style="display:flex;align-items:center;gap:14px">
      <div style="width:52px;height:52px;background:rgba(34,197,94,.1);border-radius:18px;
        display:flex;align-items:center;justify-content:center;font-size:26px">🎁</div>
      <div>
        <h4 style="font-weight:900;font-size:16px;margin-bottom:2px">مكافأة مجانية</h4>
        <p style="font-size:10px;opacity:.4;font-weight:700;text-transform:uppercase;letter-spacing:.1em">+200 عملة يومياً</p>
      </div>
    </div>
    <button onclick="window.claimFreeCoins()" id="btn-free-coins"
      style="background:#22c55e;color:#fff;padding:10px 18px;border-radius:14px;
      font-weight:900;border:none;border-bottom:3px solid #16a34a;cursor:pointer;
      font-family:'Tajawal',sans-serif;font-size:13px;white-space:nowrap">احصل 🎁</button>
  </div>`;
}
  ...
  AVATAR_FRAMES.forEach(frame=>{
    ...
    el.innerHTML=`
      <div class="frame-preview">
        <img src="${window.gameData.avatar}" style="width:54px;height:54px;border-radius:50%;object-fit:cover;${frame.style||''}">
      </div>
...
function renderThemesShop(c){
  c.innerHTML=`<div style="margin-bottom:16px">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
      ${ACCENT_COLORS.map(c=>`
        <div onclick="window.gameData.accentColor='${c.val}';window.updateUI();window.saveData();renderColorPicker();window.showToast('🎨 ${c.name}')"
          style="background:linear-gradient(135deg,${c.val},${c.val2});border-radius:18px;
          padding:20px 12px;text-align:center;cursor:pointer;transition:.2s;
          border:3px solid ${window.gameData.accentColor===c.val?'#fff':'transparent'}"
  ...
  $('player-card-content').innerHTML=`
    <div class="player-card">
      ...
      <div style="text-align:center;margin-bottom:14px">
        <span class="card-rank">${d.rank}</span>
      </div>
      <div class="card-stats">
        <div class="card-stat">
          <span class="card-stat-val">${d.level}</span>
          <span class="card-stat-lbl">المستوى</span>
        </div>
        <div class="card-stat">
          <span class="card-stat-val">${d.stats?.correctAnswers||0}</span>
          <span class="card-stat-lbl">صحيحة</span>
        </div>
        <div class="card-stat">
          <span class="card-stat-val">${d.stats?.maxStreak||0}</span>
          <span class="card-stat-lbl">أعلى سلسلة</span>
        </div>
      </div>
      ...
      <div style="text-align:center;margin-top:12px;font-size:11px;font-weight:700;color:rgba(255,255,255,.3)">
        شغل مخك · Ultra 4.0
      </div>
    </div>`;
  openModal('card');
...
window.sharePlayerCard = async ()=>{
  ...
  if(navigator.share){
    try{ await navigator.share({title:'بطاقتي في شغل مخك',text}); }
    catch(e){}
  } else {
    await navigator.clipboard.writeText(text).catch(()=>{});
    window.showToast('📋 تم نسخ البطاقة!');
  }
};
...
window.claimFreeCoins=()=>{
  ...
  $('btn-free-coins').innerText='✅ تم اليوم';
  $('btn-free-coins').disabled=true;
  window.playSound('snd-buy');
...
window.resetGame=()=>{
  window.showConfirmDialog({
    icon:'🗑️', title:'مسح البيانات',
    msg:'سيتم تصفير كل شيء نهائياً\nهل أنت متأكد؟',
    okText:'امسح كل شيء', okClass:'danger',
    onOk:async()=>{
      if(window.currentUser&&window.db&&window.firebaseReady){
        try{
          await setDoc(doc(window.db,'artifacts',window.appId,'users',window.currentUser.uid,'profile','data'),
            window.DEFAULT_DATA?window.DEFAULT_DATA():{coins:500,xp:0,level:1});
...
window.toggleSettings = ()=>{
  const p=$('settings-panel'), a=$('settings-arrow'), d=$('settings-dot');
  const open=p.classList.toggle('open');
  if(a) a.style.transform=open?'rotate(90deg)':'';
  if(d) d.style.opacity=open?'1':'0';
};
window.navTo('home');
window.updateUI();
// Build room cat select on load
setTimeout(()=>{
  const sel=$('room-cat-select'); if(!sel) return;
  Object.keys(categoryConfig).forEach(k=>{
    sel.innerHTML+=`<option value="${k}">${categoryConfig[k].icon} ${categoryConfig[k].name}</option>`;
...
// open join-room modal via inline onclick
window.openJoinRoomModal = ()=>openModal('join-room');
const openModal = type=>{ $(`modal-${type}`)?.classList.add('active'); document.body.style.overflow='hidden'; };

// keyboard shortcuts
document.addEventListener('keydown', e=>{
  if(e.key==='Escape') document.querySelectorAll('.m-overlay.active,.cmod-overlay.active').forEach(m=>m.classList.remove('active'));
});
